...
void rst::rasterizer::rasterize_triangle(
  ...
  // get bounding box
  auto ext_pt_x = std::minmax_element(
      v.begin(), v.end(),
      [](const Eigen::Vector4f &lhs, const Eigen::Vector4f &rhs) {
        return lhs.x() < rhs.x();
      });
  auto ext_pt_y = std::minmax_element(
      v.begin(), v.end(),
      [](const Eigen::Vector4f &lhs, const Eigen::Vector4f &rhs) {
        return lhs.y() < rhs.y();
      });
  ...
  for (int x = min_x; x <= max_x; ++x) {
    for (int y = min_y; y <= max_y; ++y) {
      ...
      // original code in HW2 for depth interpolation with barycentric
      // interpolation auto[alpha, beta, gamma] = computeBarycentric2D(x_pos,
      // y_pos, t.v); float w_reciprocal = 1.0/(alpha / v[0].w() + beta /
      // v[1].w() + gamma / v[2].w()); float z_interpolated = alpha * v[0].z() /
      // v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
      ...
      if (insideTriangle(x_pos, y_pos, v4f) && (-zp < depth_buf[buff_ind])) {
        ...
        auto interpolated_color =
            (alpha * c[0] / v[0].w() + beta * c[1] / v[1].w() +
             gamma * c[2] / v[2].w()) *
            Z;
        // interpolate normal vector
        auto interpolated_normal =
            (alpha * n[0] / v[0].w() + beta * n[1] / v[1].w() +
             gamma * n[2] / v[2].w()) *
            Z;
        // interpolate texture coordinate
        auto interpolated_texcoords =
            (alpha * txc[0] / v[0].w() + beta * txc[1] / v[1].w() +
             gamma * txc[2] / v[2].w()) *
            Z;
        // interpolate shading coordinate
        auto interpolated_shadingcoords =
            (alpha * view_pos[0] / v[0].w() + beta * view_pos[1] / v[1].w() +
